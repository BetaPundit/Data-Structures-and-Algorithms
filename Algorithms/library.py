...
class Item:
    ...
    def __str__(self):
        return "Item name: "+self.__item_name+" Author name: "+self.__author_name+" Published year: "+str(self.__published_year)
...
class Library:
    ...
    def sort_item_list_by_author(self, new_item_list):
        return new_item_list.sort(key=lambda x: x.get_author_name())

    def add_new_items(self, new_item_list):
        sort_item_list_by_author(new_item_list + self.__item_list)
